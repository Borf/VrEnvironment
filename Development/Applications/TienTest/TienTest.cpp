...
void TienTest::preFrame(double frameTime, double totalTime)
{
	...
	if (nextSpawn <= 0)
	{
		if (spawnCount < spawnTotal)
		{
			...
			{
				vrlib::tien::Node* n = new vrlib::tien::Node("Enemy", &tien.currentRunningScene);
	...
	for (auto t : towers)
	{
		if (tien.currentRunningScene.testBodyCollision(leftHand, t->node) || tien.currentRunningScene.testBodyCollision(rightHand, t->node))
		{
	...
	bool left;
	if (newTower &&
		((left = tien.currentRunningScene.testBodyCollision(leftHand, newTower) && vive.controllers[1].triggerButton.getData() == vrlib::DigitalState::ON) ||
		(tien.currentRunningScene.testBodyCollision(rightHand, newTower) && vive.controllers[0].triggerButton.getData() == vrlib::DigitalState::ON)))
	{
	...
	if (!newTower &&
		(tien.currentRunningScene.testBodyCollision(leftHand, archerTowerButton) && vive.controllers[1].triggerButton.getData() == vrlib::DigitalState::TOGGLE_ON) ||
		(tien.currentRunningScene.testBodyCollision(rightHand, archerTowerButton) && vive.controllers[0].triggerButton.getData() == vrlib::DigitalState::TOGGLE_ON))
	{
//		archerTowerButton->getComponent<vrlib::tien::components::Transform>()->rotate(glm::vec3(0, glm::radians(90.0f), 0));
		{
			vrlib::tien::Node* n = new vrlib::tien::Node("ArcherTower", &tien.currentRunningScene);
